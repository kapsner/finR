...
compute_relative_to_ema <- function(dataset) {
  ...
  ema_timepoints <- c("200", "100", "50" , "21")
...
query_data <- function(ric, src, from = Sys.Date() - 500, to = Sys.Date() - 1) {
  success <- FALSE
  counter <- 0
