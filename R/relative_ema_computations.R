...
compute_close_relative_to_ema <- function(dataset) {
  cutoff_date <- Sys.Date() - 1 - 366/2

  xts_datatable <- compute_ema(dataset)

  rel_to_ema <- xts_datatable[
    ,
    relative_distance(.SD, get("close")),
...
relative_to_ema <- function(ric, src) {
  ...
  ret_dat <- compute_close_relative_to_ema(dataset)
